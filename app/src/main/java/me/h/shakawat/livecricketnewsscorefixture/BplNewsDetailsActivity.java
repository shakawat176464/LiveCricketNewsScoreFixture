...
public class BplNewsDetailsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_bpl_news_details);
        ...
        newses = database.getReference("BplNews");
        ...
        if (getIntent()!=null)
            bplNewsId=getIntent().getStringExtra("BplNewsId");
        if (!bplNewsId.isEmpty())
            getDetailsNews(bplNewsId);

    }

    private void getDetailsNews(String newsID) {
        newses.child(bplNewsId).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                currentNews=dataSnapshot.getValue(BplNews.class);
